...
class KeycloakOAuth2(BaseOAuth2):  # pylint: disable=abstract-method
    ...
    def public_key(self):
        ...
        if self.setting("PUBLIC_KEY") is not None:
            return "\n".join(
                [
                    "-----BEGIN PUBLIC KEY-----",
            ...
            with urllib.request.urlopen(self.setting("SOCIAL_AUTH_BRAC_OIDC_ENDPOINT")) as url:
                public_key = json.load(url)['public_key']
            return "\n".join(
                [
                    "-----BEGIN PUBLIC KEY-----",
                    public_key,
    ...
    def user_data(
        ...
        return jwt.decode(
            ...
            options={"verify_aud": False, "verify_iat": False, "verify_signature": True},
        )
...
class KeycloakOAuth2_BRAC(KeycloakOAuth2):
    name = 'brac'

class KeycloakOAuth2_EXTERNAL(KeycloakOAuth2):
    name = 'external'